...
function MyCollection() {
  ...
  useEffect(() => {
    ...
    axios.get('http://localhost:4000/cars/mycollection', { headers })
      .then((response) => {
        setCollection(response.data.mycollection);
      })
      .catch((error) => {
        console.error('Error fetching favorites cars:', error);
      });
  }, [fresh]);
  ...
  return (
    <section className="container-fluid d-flex flex-column min-vh-100">
      <Navbar/>
      <div className="text-spyellow text-center py-3">
        <p className="text-chiaroscura display-5 my-0">My Unique Collection</p>
        <p className="text-chiaroscura h4">Of Classic Cars</p>
      </div>
      <div className="row mb-5">
      {
        collection.map((car) => (
          <div key={car.id} className="col-md-6 col-sm-6 ">
            ...
            <div className="bg-spyellow text-spdark text-center p-3">
              ...
              <div className="row">
                ...
                <div className="col-6 d-flex justify-content-end">
                  ...
                  {parseInt(userId) === car.userId ? (<a onClick={() => handleDelete(car.id)} to={`/update-car/${car.id}`} className="mx-1">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="#2A2C41" className="two-icons">
